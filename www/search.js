...
module.exports = async function handler(req, res) {
  ...
  if (req.query.id) {
    ...
    try {
      ...
      const r  = await fetch(`${RIDB}/campgrounds/${id}?full=true&apikey=${apiKey}`, { headers, signal: ac.signal });
      clearTimeout(t);
      ...
      return res.status(502).json({ error: 'RIDB fetch failed', detail: err.message });
    }
  }

  // ── Text / state search ───────────────────────────────────────
  // Use full=false for speed — cards only need name/price/lat/lng;
  // full detail data is fetched on demand when a camp is opened.
  const q = (req.query.q || '').trim().slice(0, 100);
  if (!q) return res.status(400).json({ error: 'q param required' });

  const stateCode = STATES[q.toLowerCase()];
  const params = new URLSearchParams({ query: q, limit: '25', full: 'false', apikey: apiKey });
  if (stateCode) params.set('state', stateCode);

  try {
    const ac = new AbortController();
    const t  = setTimeout(() => ac.abort(), 8000);
    const r  = await fetch(`${RIDB}/campgrounds?${params}`, { headers, signal: ac.signal });
    clearTimeout(t);
